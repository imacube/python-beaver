        ...
        for absname in files:
            try:
                ...
                if not stat.S_ISREG(st.st_mode):
                    continue
                elif int(self._beaver_config.get('ignore_old_files') > 0 \
                        and datetime.datetime.fromtimestamp(st.st_mtime) < (datetime.datetime.today() - datetime.timedelta(days=int(self._beaver_config.get('ignore_old_files')))):
